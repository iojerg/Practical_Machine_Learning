...
The train set contains `r print(obs_train)` observations and `r print(var_train)` variables. It is quite large data set, it will be split in two subsets, one used for training the model (70%) and the other for testing it (30%) and assess the model performance with new inputs.  
...
The number of predictors in the reduced data set is **`r n_var`**. Now, we can build the model using as an outcome the variable classe and the remaining variables as predictors.  
...
To reduce the number of predictors form 159 to `r n_var` has performed really well decreasing the computing time and obtaining a very high accuracy predicting model.
